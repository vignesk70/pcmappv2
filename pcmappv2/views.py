...
def email(memname, paymdate):
    ...
    # reverse('pcmappv2:sccheck_detail',args=[str(request.pk)])
    message = subject + '. Payment date: ' + \
        date_format(paymdate) + '. Please login to the admin site to validate'
...
def email_renew(request):
    ...
    # reverse('pcmappv2:sccheck_detail',args=[str(request.pk)])
    message = subject + '. Payment date: ' + \
        date_format(request.payment_date) + \
        '. Please login to the admin site to validate'
    email_from = settings.EMAIL_HOST_USER
    recipient_list = ['info@peugeotclubmalaysia.com', ]
    send_mail(subject, message, email_from, recipient_list)
    return True
...
def email_renewal_notice(member):
    subject = "Your membership is(was) due on " + \
        (member.member_expiry_date).strftime("%d %b %Y")
...
class SCCheck(LoginRequiredMixin, generic.FormView):
    ...
    def form_valid(self, form):
        carid = Car.objects.filter(
            car_reg_no=form.cleaned_data['car_reg_no']).latest('id')
...
class MemberArea(LoginRequiredMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['payment'] = Payment.objects.filter(
            payment_car_reg_no__owner=self.request.user)
        context['cars'] = Car.objects.filter(
            member_id=get_object_or_404(Member, owner=self.request.user))
        return context
...
class PCMMemberExpiring(LoginRequiredMixin, generic.ListView):
    ...
    q1 = Member.objects.select_related().filter(member_expiry_date__year=date.today().year,
                                                member_expiry_date__month=date.today().month, member_status=True).order_by('member_expiry_date')
    q2 = Member.objects.select_related().filter(member_expiry_date__lte=date.today(),
                                                member_status=True).order_by('member_expiry_date')
...
class LineChartJSONView(BaseLineChartView):
    allcars = Car.objects.all().annotate(
        total=Count('car_model')).order_by('car_model')

    def get_labels(self):
        """Return 7 labels for the x-axis."""
        allcars = Car.objects.all().order_by('car_model').values(
            'car_model').distinct().annotate(total=Count('car_model'))
        label = []
        for x in allcars:
            label.append(x['car_model'])
        # ["January 18", "February", "March", "April", "May", "June", "July"]
        return label
...
line_chart = generic.TemplateView.as_view(
    template_name='pcmappv2/line_chart.html')
